...
export default withRoute(async (req: VercelRequest, res: VercelResponse) => {
  ...
  if (req.method === 'PATCH') {
    ...
    const stripeCustomerId = body.stripe_customer_id !== undefined ? optionalString(body.stripe_customer_id) : existing.stripe_customer_id
    ...
    const rows = await sql`
      ...
      set name = ${name}, email = ${email}, phone = ${phone}, title = ${title},
          notes = ${notes}, company_id = ${companyId}, is_active_client = ${isActiveClient},
          stripe_customer_id = ${stripeCustomerId}, updated_at = now()
      where id = ${id}
      returning *
    `
